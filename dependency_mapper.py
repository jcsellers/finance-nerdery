...
def visualize_dependency_map(
    dependency_map, file_mapping, env_variables, output="dependency_map.png"
):
...
if __name__ == "__main__":
    ...
    shared_envs = {
        var: locations
        for var, locations in env_variable_usage.items()
        if len(locations) > 1
    }

    # Visualize the dependency map with shared environment variables
    visualize_dependency_map(
        resolved_map, file_mapping, shared_envs, output="tools/dependency_map.png"
    )
