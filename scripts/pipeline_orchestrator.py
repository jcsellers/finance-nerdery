...
def main():
    ...
    datasets = {ticker: f"../data/aligned/{ticker}_cleaned.csv" for ticker in yahoo_symbols}
    datasets.update({symbol: f"../data/aligned/{symbol}.csv" for symbol in fred_symbols})
    datasets.update({"SYN_LINEAR": "../data/aligned/SYN_LINEAR_cleaned.csv", "SYN_CASH": "../data/aligned/SYN_CASH_cleaned.csv"})
