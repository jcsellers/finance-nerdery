...
from synthetic_dataset_generator import generate_linear_trend, generate_cash_dataset
...
def setup_test_environment():
    ...
    for folder in [TEST_ECONOMIC_DIR, TEST_ALIGNED_DIR, os.path.dirname(TEST_DB_PATH)]:
...
def test_align_datasets(setup_test_environment):
    ...
    df = pd.DataFrame({"Date": dates, "Open": 1.0, "Close": 1.0, "High": 1.0, "Low": 1.0, "Volume": 1})
...
def test_create_sqlite_db(setup_test_environment):
    ...
    dates = pd.date_range(start="2023-01-01", end="2023-01-10")
    df = pd.DataFrame({"Date": dates, "Open": 1.0, "Close": 1.0, "High": 1.0, "Low": 1.0, "Volume": 1, "ticker": "TEST"})
